...
- [x] **GA4 tracking on all pages** — GA4 measurement ID `G-TX1X4Y0L7Y` present on all 36 HTML files (old agency ID `G-5TKMTRMEZB` removed May 2026)
...
Only the Seven Roofing-owned GA4 property fires on all pages:

| GA4 Property | Measurement ID | Owner | Status |
|-------------|---------------|-------|--------|
| **Seven Roofing owned** | `G-TX1X4Y0L7Y` | You | ✅ Active |
| ~~OLD (SEO Agency)~~ | ~~`G-5TKMTRMEZB`~~ | Old agency | ❌ Removed — May 2026 |

### How It Works

GA4 and GTM are both deferred until user interaction (scroll, click, touch, mousemove, keydown) or a 3.5-second timeout. This avoids blocking the critical rendering path:

```html
<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments)}</script>
<script>(function(){var d=false;function l(){if(d)return;d=true;
  var s=document.createElement('script');s.async=true;
  s.src='https://www.googletagmanager.com/gtag/js?id=G-TX1X4Y0L7Y';
  s.onload=function(){gtag('js',new Date());gtag('config','G-TX1X4Y0L7Y')};
  document.head.appendChild(s);
  var f=document.getElementsByTagName('script')[0],
      j=document.createElement('script');j.async=true;
  j.src='https://www.googletagmanager.com/gtm.js?id=GTM-N2VL4MSD';
  f.parentNode.insertBefore(j,f)}
['scroll','click','touchstart','mousemove','keydown'].forEach(function(e){
  window.addEventListener(e,l,{once:true,passive:true})});
setTimeout(l,3500)})();</script>
```

This ensures:
- GA4 fires on every page (deferred for performance)
- `form_submit` events from `dataLayer.push()` in `form-handler.js` fire correctly
...
WP Backup/
270.7 MB
Safe to archive. Not used by the build. Contains full WordPress installation for reference only.
seven_roofing website back-up/
3.5 MB
Safe to archive. Manual HTML snapshots of individual pages — not used by the build.
Screaming Frog SEO Spider/
0.1 MB
Safe to archive. Pre-migration SEO crawl CSV — not used by the build.
Documentation/
0.1 MB
Safe to archive. Reference docs only — not used by the build.
node_modules/
19.7 MB
Don't archive. Needed for npm run build. (Can always be restored with npm install.)
HTTrack website download/
21.1 MB
KEEP — this is the source. build-site.js reads from here.
build/
12.9 MB
KEEP — this is what you upload. Regenerated by npm run build.